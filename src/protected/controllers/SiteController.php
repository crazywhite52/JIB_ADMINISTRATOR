...
class SiteController extends Controller
{
	...
	public function actionIndex($status=null)
	{
		...
		$access = Yii::app()->request->cookies['cookie_user_access']->value;
		if($access!=1){
			$this->redirect(array("Login/in"));
		}
        //$status=$_GET['status'];
	...
	public function actionJobdel()
	{
		if(!empty($_POST)){
			$id=$_POST["id"];
		Yii::app()->msystem->createCommand("DELETE FROM es_title WHERE id='$id'")->execute();
		echo "Message sent!";
		}
	}

	public function actionChatbox()
    {
    	$this->render("chatbox");
    }
